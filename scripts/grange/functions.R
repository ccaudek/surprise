...
# this is required because the output from Gorilla.sc contains additional
# information that is not required for the analysis
tidy_flanker <- function(data){
  ...
  n_subj <- length(unique(df$subj_id))
  for (i in 1:n_subj) {
    one_subj <- df %>% 
  ...
  n_subj <- length(unique(thedat$subj_id))
  for (i in 1:n_subj) {
    one_subj <- thedat %>% 
      dplyr::filter(subj_id == i)
  ...
  # For the Grange analysis, it is necessary to have: 
  # Participant.Public.ID, congruency, Reaction.Time, Correct
...
#------------------------------------------------------------------------------
### get the accuracy per condition per participant for the flanker data

get_flanker_accuracy <- function(flanker_data, overall = TRUE){
  
  # get the list of unique participants
  # participants <- na.exclude(unique(flanker_data$Participant.Public.ID))
  
  flanker_data <- flanker_data[!is.na(flanker_data$correct), ]
  ...
  if(overall == TRUE){
    ...
    flanker_accuracy <- flanker_data %>% 
      group_by(subj_id, is_congruent_trial) %>% 
      summarise(accuracy = sum(correct) / length(correct))
  }
