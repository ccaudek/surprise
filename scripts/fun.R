# ------------------------------------------------------------------------------
# Custom functions for the surprise project
# 
# Corrado Caudek
...
#'
prep_data <- function(df, IS_CONGR) {
  
  subdf <- df %>%
    dplyr::filter(is_congruent_trial == IS_CONGR)
  
  # remove rows with NA on the rt variable
  thedat <- subdf[!is.na(subdf$rtTukey), ]
  
  # create data in proper format for choiceRT_ddm()
  thedat$subjID <- as.numeric(factor(thedat$subject_name))
...
#' @return output a data.frame
#'
manage_errors_by_subj <- function(thedat, PROCEDURE) {
